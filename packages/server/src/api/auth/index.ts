...
import { loginSchema, registerSchema } from '@tidify/common';
...
import {SESSION_USERID_PREFIX} from '../../constants';
...
router.post("/login", async (req: Request, res: Response) => {
    ...
    const user = await User.findOne({where: {email: validatedObj.email}});
    if(!user)
    ...
    if(!user.verified) {
        ...
        sendVerificationEmail({to: user.email}, await genConfirmationCode(user.id));
        return res.status(400).json({ message: 'Please verify your account first', success: false, redirect: true});
    }
    
    const userSession = {userId: user.id, username: user.username, role: user.role}

    // add session
    req.session.user = userSession;
    if (req.sessionID) {
        await redisClient.lpush(`${SESSION_USERID_PREFIX}${user.id}`, req.sessionID);
      }

    return res.status(200).json({ data: userSession, message: 'Successfully logged in!', success: true})
});
...
router.post("/logout", async (req: Request, res: Response) => {
    await removeAllUsersSessions(req.session.user!.userId, redisClient);
    req.session.destroy(async (err) => {
        if(err) throw err;
    });
    return res.status(200).clearCookie("qid").json({message: 'Successfully logged out!', success: true});
});
